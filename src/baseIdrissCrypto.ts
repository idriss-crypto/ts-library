...
import { type provider, type TransactionReceipt } from 'web3-core'
import { BigNumber, type BigNumberish } from 'ethers'

import { TwitterNameResolver } from './twitterNameResolver'
import { type ResolveOptions } from './types/resolveOptions'
...
import { type MultiSendToHashTransactionReceipt, type SendToHashTransactionReceipt } from './types/sendToHashTransactionReceipt'
import { type TransactionOptions } from './types/transactionOptions'
...
export abstract class BaseIdrissCrypto {
  ...
  private readonly idrissRegistryContractPromise
  private readonly idrissReverseMappingContractPromise
  private readonly idrissSendToAnyoneContractPromise
  private readonly priceOracleContractPromise
  private readonly tippingContractPromise
  private readonly twitterNameResolver: TwitterNameResolver
  ...
  public async resolve (input: string, options: ResolveOptions = {}): Promise<Record<string, string>> {
    const identifier = await this.transformIdentifier(input)
    const foundMatchesPromises: Record<string, Promise<string>> = {}
    for (const [network, coins] of Object.entries(BaseIdrissCrypto.getWalletTags())) {
      ...
      for (const [coin, tags] of Object.entries(coins)) {
        ...
        for (const [tag, tag_key] of Object.entries(tags)) {
          if (tag_key) {
            foundMatchesPromises[tag] = this.digestMessage(identifier + tag_key).then(async digested => await this.callWeb3GetIDriss(digested))
    ...
    for (const [tag, promise] of Object.entries(foundMatchesPromises)) {
      try {
  ...
  public async vote (
    ...
    result = await this.callWeb3Vote(encodedVote, asset, roundContractAddress, transactionOptions)
  ...
  protected async transformIdentifier (input: string): Promise<string> {
    ...
    if (inputType === null) {
      ...
      if (identifier == 'Not found') { throw new Error('Twitter handle not found.') }
    }
  ...
  public async getHashForIdentifier (identifier: string, walletType: Required<ResolveOptions>, claimPassword: string): Promise<string> {
    ...
    return await this.generateHashWithPassword(hash, claimPassword)
  }
  ...
  private async callWeb3Vote (encodedVote: string, asset: AssetLiability, roundContractAddress: string, transactionOptions: TransactionOptions): Promise<TransactionReceipt> {
  ...
  private async getVotingMethod (params: VotingParams): Promise<any> {
    let method: any
    const votingContract = await this.generateVotingContract(params.roundContractAddress)
  ...
  private async callWeb3multiSendToAnyone (params: SendToAnyoneParams[], transactionOptions: TransactionOptions): Promise<MultiSendToHashTransactionReceipt> {
    ...
    for (const param of params) {
      ...
      beneficiaryClaims.push({ beneficiary: newParam.hash!, claimPassword, claimUrl })
  ...
  private async callWeb3SendToAnyone (hash: string, beneficiary: string, asset: AssetLiability,
    message: string, transactionOptions: TransactionOptions): Promise<{ transactionReceipt: TransactionReceipt, claimUrl: string, claimPassword: string }> {
  ...
  private async generateVotingContract (_roundAddr: string) {
    return new (await this.web3Promise).eth.Contract(
  ...
  protected static getWalletTags (): Record<string, Record<string, Record<string, string>>> {
    return {
      evm: {
        ETH: {
  ...
  protected static async generateWeb3 (web3: Promise<any>, url: string, provider?: provider) {
    return await web3
      .then(x => x.default)
      .then(Web3 => new Web3(
  ...
  public async reverseResolve (address: string) {
    const result = await this.callWeb3ReverseIDriss(address)
    if (+result) {
